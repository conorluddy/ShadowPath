...
//   "toggle" — any subset is active, each independently on/off, in declared
//              order. An optional `defaultEnabled` whitelist sets which start on;
//              omit it to start every option on.
...
export function defaultPipelineState(definition) {
  ...
  for (const kind of STAGE_KINDS) {
    ...
    if (stage.mode === "toggle") {
      ...
      state[kind] = { enabled: Object.fromEntries(stage.options.map((id) => [id, startsOn(id)])) };
    } else {
